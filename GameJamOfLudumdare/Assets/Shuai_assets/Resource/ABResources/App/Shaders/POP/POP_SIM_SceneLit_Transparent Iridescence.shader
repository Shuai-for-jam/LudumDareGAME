Shader "Universal Render Pipeline/POP/SIM/Scene/Scene Lit Transparent Iridescence"
{
    Properties
    {
        ...
        _RepeatNoise("Repeat Noise", Range(0, 10)) = 1
        _NoiseIntensity("Noise Intensity", Range(0, 1)) = 0.1
        _LightSpan("Light Span",Range(0, 1)) = 0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _LightSpan, _LightOffset, _Saturation, _Brightness;
            real4 _EnvironmentTex_HDR;
            half _EnvironmentRotation;
            half _EnvReflectionIntensity;
            half _RepeatNoise, _NoiseIntensity;
            CBUFFER_END
            ...
            inline void InitializeStandardLitSurfaceData_SIM(float2 uv, float3 normalWS, float3 positionWS, out SurfaceData outSurfaceData, float3 worldPos)
            {
                ...
                //outSurfaceData.albedo = albedoAlpha.rgb;
                 half4 noise = SAMPLE_TEXTURE2D(_NoiseMap, sampler_NoiseMap, uv * _RepeatNoise) * _NoiseIntensity;
                half3 viewDirWS = GetWorldSpaceNormalizeViewDir(positionWS);
                float3 normalize_normalWS = normalize(normalWS);
                float3 normalize_viewDirWS = normalize(viewDirWS);
                float NdotV = dot(normalWS, viewDirWS) + noise.r;
                float H = (1 - NdotV) * _LightSpan + _LightOffset;
                float3 HSV = float3(H, _Saturation, _Brightness);
                float3 RGB = pow(HSVtoRGB(HSV), 2.2);
                outSurfaceData.albedo = RGB;
                                                  ...
                                                  half occlusion)
            {
                ...
                //half4 encodedIrradiance = half4(
                 ...
                 half4 encodedIrradiance = half4(SAMPLE_TEXTURECUBE_LOD(_EnvironmentTex, sampler_EnvironmentTex, reflectVector, mip));

                #if defined(UNITY_USE_NATIVE_HDR)
                    irradiance = encodedIrradiance.rgb;
                #else
                irradiance = DecodeHDREnvironment(encodedIrradiance, _EnvironmentTex_HDR);
                                         ...
                                         half3 normalWS, half3 viewDirectionWS)
            {
                ...
                reflectVector = mul(reflectVector, rotationMatrix);
                half NoV = saturate(dot(normalWS, viewDirectionWS));
                half fresnelTerm = Pow4(1.0 - NoV);
                half3 indirectDiffuse = bakedGI;
                half3 indirectSpecular = GlossyEnvironmentReflection_SIM(
                    reflectVector, positionWS, brdfData.perceptualRoughness, 1.0h) * _EnvReflectionIntensity;
            ...
            half4 LitPassFragment(Varyings input) : SV_Target
            {
                ...
                InitializeStandardLitSurfaceData_SIM(input.uv, input.normalWS, input.positionWS, surfaceData, input.positionWS);
                InputData inputData;
        ...
        Pass
        {
            ...
            Varyings vert(Attributes IN)
            {
                ...
                OUT.normalWS = TransformObjectToWorldDir(IN.normal);
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                half4 noise = SAMPLE_TEXTURE2D(_NoiseMap, sampler_NoiseMap, IN.uv * _RepeatNoise) * _NoiseIntensity;
                float3 normalize_normalWS = normalize(IN.normalWS);
                float3 normalize_viewDirWS = normalize(IN.viewDirWS);
                float NdotV = dot(normalize_normalWS, normalize_viewDirWS) + noise.r;
                float H = (1 - NdotV) * _LightSpan + _LightOffset;
                float3 HSV = float3(H, _Saturation, _Brightness);
                float3 RGB = pow(HSVtoRGB(HSV), 2.2);
