Shader "Universal Render Pipeline/POP/SIM/HoloEffect"
{
    Properties
    {
...
//        [HideInInspector][NoScaleOffset]unity_ShadowMasks("unity_ShadowMasks", 2DArray) = "" {}
        ...
        [Space(18)]
        [KeywordEnum(OFF, ON)]OUTTT("Y For Height",Float) = 1
        _HoloLensMap("HoloLens Map", 2D) = "bump" {}
        [HDR]_HoloLnesColor("HoloLens Color", Color) = (1, 1, 1, 1)
        _HoloLensTileX("HoloLens Tile X", Range(0, 10)) = 1
        _HoloLensTileY("HoloLens Tile Y", Range(0, 10)) = 1
        _HoloDitter("HoloLens Ditter", Range(0, 100)) = 10
        _HoloDitterOpacity("HoloLens Ditter Opacity", Range(0, 1)) = 0
        _HoloLensSpeed("HoloLens Speed", Range(0, 1)) = 1
        _HoloDissloveColor("Holo Disslove Color", Color) = (1, 1, 1, 1)
        [Space(18)]
        _AdjustHeight("Adjust Height", Range(0, 5)) = 1
        _AdjustWidth("Adjust Width", Range(0, 5)) = 1
        _AdjustDissolveOffset("Adjust Dissolve Offset", Range(0, 1)) = 0
        [Space(18)]
        _HoloLensRegion("HoloLens Region", Range(0, 1)) = 1
    ...
    SubShader
    {
        ...
        Tags{"Queue"="Transparent" "RenderType" = "Transparent" "RenderPipeline" = "UniversalPipeline" "UniversalMaterialType" = "Lit" "IgnoreProjector" = "True" "ShaderModel"="3.0"}
        LOD 300
        ...
        Pass
        {
            ...
            #pragma multi_compile _OUTTT_OFF OUTTT_ON
            ...
            half _HoloLensTileY;
            half _HoloLensSpeed;
            half4 _HoloLnesColor;
            half _HoloLensRegion;
            half _AdjustHeight;
            half _AdjustWidth;
            half _HoloDissloveEdge;
            half4 _HoloDissloveColor;
            half _AdjustDissolveOffset;
            half _HoloDitter, _HoloDitterOpacity;
            half _SmoothnessRemappingLow, _SmoothnessRemappingHigh, _SmoothnessRemappingLevelLow,
                 _SmoothnessRemappingLevelHigh, _MetallicRemappingLow, _MetallicRemappingHigh,
                 _MetallicRemappingLevelLow, _MetallicRemappingLevelHigh;
            CBUFFER_END
            ...
            TEXTURE2D(_HoloLensMap);        SAMPLER(sampler_HoloLensMap);
            ...
            Varyings LitPassVertex(Attributes input)
            {
                ...
                output.hightOS.x = input.positionOS.x;
                #ifdef OUTTT_ON
                output.hightOS.y = input.positionOS.y;
                #else
                output.hightOS.y = input.positionOS.z;
                #endif
            ...
            half4 LitPassFragment(Varyings input) : SV_Target
            {
                ...
                // add Hololens Effect
                half height_remapping = input.hightOS.y/_AdjustHeight;
                half width_remapping = input.hightOS.x/_AdjustWidth * 0.5 + 0.5;
                half2 hololens_UV01 = half2(0, height_remapping);
                hololens_UV01.y *= half2(_HoloLensTileX, _HoloLensTileY);
                hololens_UV01.y +=  _Time.y * _HoloLensSpeed;
                half2 hololens_edge_UV = half2(width_remapping, height_remapping - _HoloLensRegion + _AdjustDissolveOffset);
                half hololens_edge = 1 - step(SAMPLE_TEXTURE2D(_HoloLensMap, sampler_HoloLensMap, hololens_edge_UV).g, 0.1f);
                
               
    
                half4 holoLensCol = SAMPLE_TEXTURE2D(_HoloLensMap, sampler_HoloLensMap, hololens_UV01).r * _HoloLnesColor;
                half clip = step(height_remapping, _HoloLensRegion);
                half clipEdge = saturate((clip + hololens_edge) * 10);
                color.rgb = lerp(holoLensCol.rgb, color.rgb + hololens_edge, clipEdge);
                color.a = lerp( holoLensCol.a * saturate(abs(sin(_Time.y * _HoloDitter)) + _HoloDitterOpacity) * surfaceData.alpha, surfaceData.alpha, clipEdge)*_BaseColor.a;
                return color;
            }
            ENDHLSL
        }
