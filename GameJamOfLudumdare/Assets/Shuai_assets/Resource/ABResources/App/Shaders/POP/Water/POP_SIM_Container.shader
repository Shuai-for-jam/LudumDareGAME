Shader "Universal Render Pipeline/POP/SIM/Scene/Scene Container"
{
    Properties
    {
        [Header(__ Distortion __)][Space(30)]
        ...
        _DistortionTill("Distortion Till", Range(0, 20)) = 1
        ...
        _NrmRepeat("Normal Repeat", Range(0.01, 10)) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 _FoamColor;
            float _NrmRepeat;
            half _DistortionIntensity, _DistortionSpeed, _DistortionTill;
            CBUFFER_END
            ...
            struct v2f
            {
                ...
                float2 uv_distortion : TEXCOORD8;
                #endif
                UNITY_VERTEX_INPUT_INSTANCE_ID
            };
            ...
            v2f vert(attributes i)
            {
                ...
                ///// UV传递 /////
                o.uv.xy = i.uv.xy;
                o.uv.zw = i.uv.xy * _NrmRepeat;
                o.uv_distortion = i.uv.xy;
            ...
            half4 frag(v2f i) : SV_Target
            {
                ...
                half3 nrm1 = UnpackNormal(tex2D(_NrmTex, i.uv.xy * _NrmRepeat)); // 法线纹理1
                ...
                // 获取世界法线纹理
                half3 N = normalize(half3(dot(i.TtoW0.xyz, nrm1), dot(i.TtoW1.xyz, nrm1), dot(i.TtoW2.xyz, nrm1)));
                ...
                //获取海底颜色
                half3 distortionCol = tex2D(_DistortionTex, frac(i.uv * _DistortionTill + _DistortionSpeed * _Time.y));
                half2 screenUV_Distortion = screenUV + distortionCol.xy * _DistortionIntensity;
                half3 cameraColorTexture = SampleSceneColor(screenUV_Distortion);
                ...
                ///基础颜色
                half3 baseColor = cameraColorTexture * _BaseColor;
