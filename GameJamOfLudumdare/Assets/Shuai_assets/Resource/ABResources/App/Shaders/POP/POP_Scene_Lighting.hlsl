#ifndef UNIVERSAL_POP_SCENE_LIGHTING_INCLUDED
#define UNIVERSAL_POP_SCENE_LIGHTING_INCLUDED

#include "POP_Scene_BRDF.hlsl"
#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Debug/Debugging3D.hlsl"
#include "POP_Scene_GlobalIllumination.hlsl"
#include "POP_Scene_RealtimeLights.hlsl"
#include "POP_Scene_AmbientOcclusion.hlsl"
...
LightingData CreateLightingData(InputData inputData, POP_SurfaceData surfaceData)
{
...
half3 CalculateBlinnPhong(Light light, InputData inputData, POP_SurfaceData surfaceData)
{
...
//add by zjy
half3 CalculateSSS(Light light, InputData inputData, POP_SurfaceData surfaceData)
{
    float3 back_dir = -normalize(light.direction + inputData.normalWS * _SSSScattering);
    half3 VdotB = max(0.0, dot(back_dir, inputData.viewDirectionWS));
    half3 backlight = max(0.0, pow(VdotB, _SSSPower)) * _SSSIntensity * _SSSColor.rgb * ( surfaceData.thickness) * light.color;
    return saturate(backlight);
         
}

half3 CalculateFresnel(Light light, InputData inputData)
{
    half fresnelTerm = max(0, dot(inputData.viewDirectionWS, inputData.normalWS));
    half3 fresnel = _FresnelIntensity * saturate(pow(1 - fresnelTerm, _FresnelPow)) * _FresnelColor.rgb;
    return fresnel;
}
//
...
/// PBR lighting...
////////////////////////////////////////////////////////////////////////////////
half4 UniversalFragmentPBR(InputData inputData, POP_SurfaceData surfaceData)
{
    ...
    POPAmbientOcclusionFactor aoFactor = CreateAmbientOcclusionFactor(inputData, surfaceData);
    ...
    if (IsMatchingLightLayer(mainLight.layerMask, meshRenderingLayers))
    {
        ...
        //add by zjy
        half3 sss = CalculateSSS(mainLight, inputData, surfaceData);
        lightingData.mainLightColor += sss;
        #ifdef OUT_ON
            half3 fresnel = CalculateFresnel(mainLight, inputData);
            lightingData.mainLightColor += fresnel;
        #endif
        //return half4( CalculateSSS(mainLight, inputData, surfaceData), 1);
...
/// Phong lighting...
////////////////////////////////////////////////////////////////////////////////
half4 UniversalFragmentBlinnPhong(InputData inputData, POP_SurfaceData surfaceData)
{
    #if defined(DEBUG_DISPLAY)
    half4 debugColor;
...
/// Unlit
////////////////////////////////////////////////////////////////////////////////
half4 UniversalFragmentBakedLit(InputData inputData, POP_SurfaceData surfaceData)
{
